...
'''
@author: liuzd
@time: 2017/8/15 09:20
@Function: 问题类别与处理方式关联分析并分区
'''
import pandas as pd
...
def dealData(df):
    ...
    bWay = df['Deal_Type_Code']
    # print wCategory, bWay
...
def summary(cursor, startTime, endTime):
    sql = "SELECT m.ISSUE_REGION_FLAG,m.Issue_Region_Code4,m.Region_Code,m.Petition_Date,m.Petition_Type_Code,m.Source_Code4,m.Object_Class_Code,m.Issue_Type_Code,m.Deal_Type_Code,e.Reality_Code" \
          ...
          "where d.valid_flag='0' and aa.Petition_Date >""'" + startTime + "'"" and aa.Petition_Date<""'" + endTime + "'"" ) m " \
                                                                                                                      "left join Petition_End_Info e on m.dOid=e.Petition_Deal_Info_Oid   where m.DEAL_TYPE_CODE != ' ' and  m.Issue_Type_Code != ' '"
    # 每次执行一次查询
    cursor.execute(sql)
...
def mainQD(cursor, modelCycle, conn, supportCity, supportArea, supportWei):
    try:
        start = time.clock()
        ...
        sqlDelete = "delete from DATAPREDICT_FPM_QD_INFO where substr(CREATE_DATE,1,10)<='" + yesTimeNyr + "'"  # 删表数据
        logging.info(" delete table  sql: %s\t" % (sqlDelete))
        cursor.execute(sqlDelete)  # 执行
        end = time.clock()  # 运行结束时间
        logging.info(u'\n问题类别与处理方式关联分析并分区存储完毕，用时：%0.2f秒' % (end - start))
        # print(u'\n问题类别与处理方式关联分析并分区存储完毕，用时：%0.2f秒' % (end - start))  # 打印运行总时间
    except Exception as e:
