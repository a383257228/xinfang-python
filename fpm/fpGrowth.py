...
def find_frequent_itemsets(transactions, minimum_support, include_support=False):
    ...
    just the itemsets.
    """
    # dict.setdefault()方法接收两个参数，第一个参数是健的名称，第二个参数是默认值。假如字典中不存在给定的键，则返回参数中提供的默认值；
    ...
    def find_with_suffix(tree, suffix):
        for item, nodes in tree.items():
            ...
            # confidence(A->B) = support_count(A并B) / support_count(A)
            if support >= minimum_support and item not in suffix:
                # New winner!
                found_set = [item] + suffix
                # ssCnt = {}
                # if len(found_set) > 1 :
                #     if not ssCnt.has_key(tuple(found_set)):
                #         ssCnt[tuple(found_set)] = 1
                #     else:
                #         ssCnt[tuple(found_set)] += 1
                # confidence =   ssCnt[tuple(found_set)] / support
                yield (found_set, support) if include_support else found_set

                # Build a conditional tree and recursively search for frequent  构建条件树并递归搜索
                # itemsets within it.
